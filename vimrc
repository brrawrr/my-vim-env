...
set statusline=%{exists(\"*fugitive#statusline\")?\"branch:\ \".fugitive#statusline():\"\"}\ %F%m%r%h%w\ %h%m%r%=%{\"[\".(&fenc==\"\"?&enc:&fenc).((exists(\"+bomb\")\ &&\ &bomb)?\",B\":\"\").\"]\ \"}(%{&ff}){%Y}\ \ [l,%v][%p%%]
...
if &fileencodings !~? "cp1251"
  set fileencodins+=cp1251
endif
" }}}

" Indicate page margins (80 and 120)
